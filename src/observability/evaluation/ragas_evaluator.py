...
from src.providers.evaluator.base_evaluator import BaseEvaluator
...
class RagasEvaluator(BaseEvaluator):
    ...
    def _build_wrappers(self) -> tuple:
        ...
        if use_azure_llm:
            ...
            llm_client = AsyncOpenAI(api_key="ollama", base_url=base_url)
        else:
            raise ValueError(
        ...
        if use_azure_emb:
            ...
            base_url = getattr(emb_cfg, "base_url", "http://localhost:11434") + "/v1"
            emb_client = AsyncOpenAI(api_key="ollama", base_url=base_url)
        else:
            raise ValueError(
